...
namespace SteamTrade.TradeOffer
{
    public class TradeOffer
    {
        ...
        public class TradeStatus
        {
            ...
            [JsonProperty("me")]
            public TradeStatusUser MyOfferedItems { get; set; }
            ...
            public bool AddMyItemByAsset(long assetid, int appid) // Checks are assumed to be done by the database
            {
                if (assetid == 0) { return false; }

                if (appid == 753)
                {
                    AddMyItem(appid, 6, assetid);
                }
                else
                {
                    AddMyItem(appid, 2, assetid);
                }

                return true;
            }

            public bool AddTheirItemByAsset(long assetid, int appid) // Checks are assumed to be done by the database
            {
                if (assetid == 0) { return false; }

                if (appid == 753)
                {
                    AddTheirItem(appid, 6, assetid);
                }
                else
                {
                    AddTheirItem(appid, 2, assetid);
                }
