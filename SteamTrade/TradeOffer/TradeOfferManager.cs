...
namespace SteamTrade.TradeOffer
{
    public class TradeOfferManager
    {
        ...
        public void FetchActiveIncoming()
        {
            try
            {
                var resp = webApi.GetActiveTradeOffers(false,true, false);
                
                foreach (var offer in resp.TradeOffersReceived)
                {
                    try
                    {
        ...
        public void DeclineAllActiveOffers()
        {
            try
            {
                var resp = webApi.GetActiveTradeOffers(true, false, false);

                foreach (var item in resp.TradeOffersSent)
                {
                    try
                    {
                        if (item.TradeOfferState == TradeOfferState.TradeOfferStateActive || item.TradeOfferState == TradeOfferState.TradeOfferStateNeedsConfirmation)
                        {
                            TradeOffer tradeOffer = null;
                            TryGetOffer(item.TradeOfferId, out tradeOffer);
            ...
            DeclineAllIncomingOffers();

        }

        public void DeclineAllIncomingOffers()
        {
            try
            {
                var resp = webApi.GetActiveTradeOffers(false, true, false);

                foreach (var item in resp.TradeOffersReceived)
                {
                    try
                    {
                        if (item.TradeOfferState == TradeOfferState.TradeOfferStateActive)
                        {
                            TradeOffer tradeOffer = null;
                            TryGetOffer(item.TradeOfferId, out tradeOffer);
