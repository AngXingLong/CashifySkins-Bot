...
namespace SteamBot
{
    public class Bot : IDisposable
    {
        ...
        {
            tradeOfferManager.DeclineAllIncomingOffers();
        }

        public void DeclineAllActiveOffers()
        {
            tradeOfferManager.DeclineAllActiveOffers();
        }

        public void FetchActiveIncomingOffers()
        {
            while (true)
            {
                tradeOfferManager.FetchActiveIncoming();
                Thread.Sleep(1000);
            }
        }
        ...
        /// </summary>
        public GenericInventory FetchInventory(SteamID steamid, int appid)
        {
            GenericInventory inventory = new GenericInventory(SteamWeb);
            if (inventory.FetchInventoryData(appid, steamid))
            {
                return inventory;
            }
        ...
        public void AcceptAllMobileTradeConfirmations()
        {
            ...
            else
            {
                ...
                {
                    CheckCookies();
                    Log.Warn("Invalid session when trying to fetch trade confirmations.");
                }
                catch(Exception e)
                {
                    Log.Warn(e.ToString());
