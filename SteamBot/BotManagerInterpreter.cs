...
namespace SteamBot
{
    ...
    public class BotManagerInterpreter
    {
        ...
        public void RedisCommand(bool proccess, ulong steamid)
        {
            SteamTrade.TradeNetwork TradeNet = new SteamTrade.TradeNetwork(1);

            if (steamid == 1) // global command
            {
                try
                {
                    if (proccess)
                    {
                        List<ulong> BotStatusList = TradeNet.BotExepectedStatusList(1);

                        foreach (ulong botsid in BotStatusList)
                        {
                            foreach (var bot in manager.ConfigObject.Bots)
                            {
                                if (bot.SteamID == botsid)
                                {
                                    manager.StartBot(bot.Username);
                                    break;
                                }
                            }
                        }
                    ...
                    else
                    {
                        foreach (var bot in manager.ConfigObject.Bots)
                        {
                            ...
                            db.ListLeftPush(Convert.ToString(bot.SteamID), -2);
                        }
                        Console.WriteLine("Shuting down All Bots");
                        System.Threading.Thread.Sleep(3 * 60 * 1000); // Wait for bot to finish task before stoping the bot

                        foreach (var bot in manager.ConfigObject.Bots)
                        {
                            manager.StopBot(bot.Username);
                        }

                        foreach (var bot in manager.ConfigObject.Bots)
                        {
                            IDatabase db = RedisNetwork.Redis.GetDatabase();
                            TradeNet.UpdateBotStatus(0, bot.SteamID);
                        }
                    }
                }
                catch (Exception e)
                {
                    Console.WriteLine("Bot Manager Error: " + e);
                }

                WorkerToManager.botsid.Clear();
                return;
            }
            else
            {
                if (proccess)
                {
                    ...
                    TradeNet.UpdateBotStatus(1, steamid);

                }
                else
                {
                    TradeNet.UpdateBotStatus(2, steamid);
                    ...
                    foreach (var bot in manager.ConfigObject.Bots)
                    {
                        ...
                        {
                            IDatabase db = RedisNetwork.Redis.GetDatabase();
                            db.ListLeftPush(Convert.ToString(steamid), -2);
                            DateTime lastReported = DateTime.UtcNow;

                            while (true)
                            {
                                if (WorkerToManager.botsid.Contains(steamid) || DateTime.UtcNow > lastReported.AddMinutes(5))
                                {
                                    manager.StopBot(bot.Username);
                                    WorkerToManager.botsid.Remove(steamid);
                                    break;
                                }
                            }
                    ...
                    TradeNet.UpdateBotStatus(0, steamid);
                }
            }
        }
